...
export function extractTimeblock(line) {
...
export function formatDisplayTime(hour) {
...
export function addTimeblock(line, startHour, startMinute, endHour, endMinute) {
...
export function removeTimeblock(line) {
  return line.replace(/^([\t]*- \[.\]\s*)\d{2}:\d{2}\s*-\s*\d{2}:\d{2}\s*/, '$1');
}

// Calculate default end time (start + 30 minutes)
export function getDefaultEndTime(startHour, startMinute) {
